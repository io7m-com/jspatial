...
public final class OctTreeRaycastResultIGenerator<A>
  implements Generator<OctTreeRaycastResultIType<A>>
{
  private final DoubleGenerator dgen;
  private final VolumeIGenerator agen;
  private final Generator<A> ogen;

  public OctTreeRaycastResultIGenerator(
    final Generator<A> in_ogen)
  {
    this.dgen = new DoubleGenerator(0.0, Double.MAX_VALUE);
    this.agen = new VolumeIGenerator(new IntegerGenerator());
    this.ogen = in_ogen;
  }

  @Override
  public OctTreeRaycastResultIType<A> next()
  {
    return OctTreeRaycastResultI.of(
