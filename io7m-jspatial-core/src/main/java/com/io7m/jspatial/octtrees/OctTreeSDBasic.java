 ...
 * </p>
 * <p>
 * As mentioned in {@link com.io7m.jspatial.quadtrees.QuadTreeBasic}, many
  ...
  OctTreeSDType<T>
{
  private final class Octant implements OctantType
  {
    ...
    private boolean insertBase(
      ...
      final SDType type)
    {
      if (OctTreeSDBasic.this.objects_all_dynamic.contains(item)) {
        return false;
      }
      if (OctTreeSDBasic.this.objects_all_static.contains(item)) {
        return false;
      }
      ...
      final SDType type)
    {
      switch (type) {
        case SD_DYNAMIC:
        {
          OctTreeSDBasic.this.objects_all_dynamic.add(item);
          this.octant_objects_dynamic.add(item);
          return true;
        }
        case SD_STATIC:
        {
          OctTreeSDBasic.this.objects_all_static.add(item);
    ...
    boolean remove(
      final T item)
    {
      if ((OctTreeSDBasic.this.objects_all_dynamic.contains(item) == false)
        && (OctTreeSDBasic.this.objects_all_static.contains(item) == false)) {
        return false;
      }
      ...
      final T item)
    {
      if (this.octant_objects_dynamic.contains(item)) {
        ...
        OctTreeSDBasic.this.objects_all_dynamic.remove(item);
        return true;
      }
      if (this.octant_objects_static.contains(item)) {
        this.octant_objects_static.remove(item);
        OctTreeSDBasic.this.objects_all_static.remove(item);
        return true;
      }
