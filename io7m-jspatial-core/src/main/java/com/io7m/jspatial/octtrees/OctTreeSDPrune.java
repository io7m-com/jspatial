...
@SuppressWarnings("synthetic-access") public final class OctTreeSDPrune<T extends OctTreeMemberType<T>> implements
  OctTreeSDType<T>
{
  private final class Octant implements OctantType
  {
    ...
    private boolean insertBase(
      ...
      final SDType type)
    {
      if (OctTreeSDPrune.this.objects_all_dynamic.contains(item)) {
        return false;
      }
      if (OctTreeSDPrune.this.objects_all_static.contains(item)) {
        return false;
      }
      ...
      final SDType type)
    {
      switch (type) {
        case SD_DYNAMIC:
        {
          OctTreeSDPrune.this.objects_all_dynamic.add(item);
          this.octant_objects_dynamic.add(item);
          return true;
        }
        case SD_STATIC:
        {
          OctTreeSDPrune.this.objects_all_static.add(item);
    ...
    boolean remove(
      final T item)
    {
      if ((OctTreeSDPrune.this.objects_all_dynamic.contains(item) == false)
        && (OctTreeSDPrune.this.objects_all_static.contains(item) == false)) {
        return false;
      }
  ...
  {
    return new OctTreeSDPrune<T>(position, size);
  }
