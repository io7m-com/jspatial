...
@SuppressWarnings("synthetic-access") public final class OctTreePruneLimit<T extends OctTreeMemberType<T>> implements
  OctTreeType<T>
{
  private final class Octant implements OctantType
  {
    ...
    {
      final int mx = OctTreePruneLimit.this.minimum_size_x;
      final int my = OctTreePruneLimit.this.minimum_size_y;
      final int mz = OctTreePruneLimit.this.minimum_size_z;
  ...
  @SuppressWarnings("boxing") private OctTreePruneLimit(
    final VectorReadable3IType in_position,
    final VectorReadable3IType in_size,
    final VectorReadable3IType in_size_minimum)
  {
    ...
    if (in_size_minimum.getZI() > in_size.getZI()) {
      final String s =
        String.format(
    ...
    this.minimum_size_x = in_size_minimum.getXI();
    this.minimum_size_y = in_size_minimum.getYI();
    this.minimum_size_z = in_size_minimum.getZI();
