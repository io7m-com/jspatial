...
@SuppressWarnings("synthetic-access") public final class QuadTreeSDBasic<T extends QuadTreeMemberType<T>> implements
QuadTreeSDType<T>
{
  final class Quadrant implements QuadrantType
  {
    ...
    private boolean insertBase(
      ...
      final SDType type)
    {
      if (QuadTreeSDBasic.this.objects_all_static.contains(item)) {
        return false;
      }
      if (QuadTreeSDBasic.this.objects_all_dynamic.contains(item)) {
        return false;
      }
      ...
      final SDType type)
    {
      switch (type) {
        case SD_DYNAMIC:
        {
          QuadTreeSDBasic.this.objects_all_dynamic.add(item);
          this.quadrant_objects_dynamic.add(item);
          return true;
        }
        case SD_STATIC:
        {
          QuadTreeSDBasic.this.objects_all_static.add(item);
    ...
    boolean remove(
      final T item)
    {
      if ((QuadTreeSDBasic.this.objects_all_dynamic.contains(item) == false)
        && (QuadTreeSDBasic.this.objects_all_static.contains(item) == false)) {
        return false;
      }
      ...
      final T item)
    {
      if (this.quadrant_objects_dynamic.contains(item)) {
        ...
        QuadTreeSDBasic.this.objects_all_dynamic.remove(item);
        return true;
      }

      if (this.quadrant_objects_static.contains(item)) {
        this.quadrant_objects_static.remove(item);
        QuadTreeSDBasic.this.objects_all_static.remove(item);
        return true;
      }
  ...
  newQuadTree(
    ...
    {
    return new QuadTreeSDBasic<T>(position, size);
    }
